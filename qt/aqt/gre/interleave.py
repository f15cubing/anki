...
# Vendored verbatim from pipeline/interleave.py (outer repo); drift-guarded by
# tests/test_interleave_sync.py. Do not edit here — edit the source and re-vendor.
...
def displacement(original, reordered) -> tuple[float, int]:
    ...
    *later* than its FSRS priority slot it ended up (the starvation direction). Pulling
    ...
    shifts = [
        max(0, new_i - fsrs_index[cid]) for new_i, (cid, _leaf) in enumerate(reordered)
    ]
    if not shifts:
        return (0.0, 0)
...
def interleave(
    queue, *, k: int = DEFAULT_K, w: int = DEFAULT_W, clusters=None
) -> InterleaveResult:
